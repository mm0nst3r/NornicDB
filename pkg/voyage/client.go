...
// Error strings contain a closed category and sanitized request diagnostics. Exact
...
func (e *Error) Error() string {
	// Public adapters render errors even when no result/report is returned.
	// Keep the known diagnostic fields at that canonical boundary. Metadata
	// contains only sanitized identifiers and numeric usage, never the request,
	// credential, response body or underlying URL-bearing transport error.
	metadata, _ := json.Marshal(e.Metadata)
	return fmt.Sprintf("voyage: %s (HTTP %d; metadata=%s)", e.Kind, e.Metadata.StatusCode, metadata)
}
...
func (e envelope) metadata(meta Metadata, requested string) (Metadata, error) {
	...
	meta.Usage = e.Usage
	// Only a bounded safe token can enter public diagnostics.
