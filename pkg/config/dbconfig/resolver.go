...
func effectiveFromGlobal(c *config.Config, m map[string]string) {
	...
	setEffective("NORNICDB_SEARCH_RERANK_TRUNCATION", boolStr(c.Features.SearchRerankTruncation))
	setEffective("NORNICDB_SEARCH_RERANK_FAILURE_POLICY", c.Features.SearchRerankFailurePolicy)
	// K-means (from Memory)
	...
	setEffective("NORNICDB_EMBED_AUTO_CHUNKING", strconv.FormatBool(c.EmbeddingWorker.AutoChunking))
