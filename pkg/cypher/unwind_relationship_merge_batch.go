...
func (e *StorageExecutor) executeUnwindRelationshipMergeBatch(
	...
	for _, row := range rows {
		...
		if relationshipMergeIdentityContainsNaN(matchProps) {
			...
			// share the batch identity cache. Defer to row-wise MERGE instead.
			return nil, false, nil
		}
		if !relationshipBatchPropertiesPreserveMatch(props, plan.merge, row) {
			// Writes are deferred until every row has been planned, so this can
			// safely route SET rel = row shapes that change MERGE identity to the
			// generic executor without leaving a partial batch behind.
			return nil, false, nil
		}
		...
		edge := &storage.Edge{
			ID:         e.newRelationshipMergeEdgeID(start.ID, end.ID, plan.merge.relType, matchProps),
		...
		if selected != nil {
			updated := &storage.Edge{
				ID:                   selected.ID,
				Type:                 selected.Type,
				StartNode:            selected.StartNode,
				EndNode:              selected.EndNode,
				Properties:           props,
				CreatedAt:            selected.CreatedAt,
				UpdatedAt:            selected.UpdatedAt,
				Confidence:           selected.Confidence,
				AutoGenerated:        selected.AutoGenerated,
				VisibilitySuppressed: selected.VisibilitySuppressed,
			}
			updates = append(updates, updated)
			existingByKey[edgeKey] = updated
			result.Rows = append(result.Rows, buildRelationshipBatchReturnRow(row, plan.returns))
...
func relationshipBatchMatchProperties(
	...
	props := make(map[string]interface{}, len(merge.keyProps))
	for _, prop := range merge.keyProps {
		if prop.literalSet {
			props[prop.propName] = normalizePropValue(prop.literal)
			continue
		}
		props[prop.propName] = normalizePropValue(row[prop.rowField])
	}
	return props
}

func relationshipBatchPropertiesPreserveMatch(
	...
	for _, prop := range merge.keyProps {
		want := prop.literal
		if !prop.literalSet {
			want = row[prop.rowField]
		}
		got, ok := props[prop.propName]
...
func relationshipBatchEdgeKey(startID storage.NodeID, endID storage.NodeID, relType string, mergeProps map[string]interface{}) string {
	normalized := make(map[string]interface{}, len(mergeProps))
	for key, value := range mergeProps {
		normalized[key] = normalizeRelationshipMergeIdentityValue(value)
	}
	return string(startID) + "\x00" + string(endID) + "\x00" + relType + "\x00" + unwindMergeKey("", normalized)
}
...
func relationshipBatchScalarEdgeKey(
	...
	for _, prop := range keyProps {
		...
		value = normalizeRelationshipMergeIdentityValue(value)
