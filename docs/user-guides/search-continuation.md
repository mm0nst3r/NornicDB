...
Built-in Badger and memory storage publish database-scoped node revisions;
...
bulk operations and namespace-prefix deletions invalidate retained populations
...
Custom/external engines without `storage.NodeMutationVersionProvider`, and
...
custom mutation paths that do not publish node revisions. Unreported changes
