...
- Prevent ranked continuation from treating short approximate, filtered, or fused batches as exhaustion. Propagate retrieval exhaustion evidence, deepen chunk candidates beyond one-shot limits, remove the arbitrary 5,000-candidate engine ceiling, and report caller-selected budget limits explicitly instead of silently ending the stream. `ranked_then_id` now deepens every query chunk, and a fixed Stage-2 rerank pool is reported as a bounded capacity error after one retrieval instead of being retried until overflow or mistaken for exhaustion. Grouped gRPC children use the `SearchHit` message so they carry the same per-hit fields as top-level hits.
